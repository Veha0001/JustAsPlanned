...
@click.option("-ks", "--keepres", is_flag=True, help="Keep original resources.")
...
def main(**kwargs):
    ...
    if args.appname:
        packer["com.prpr.musedash"]["app_name"] = args.appname
    if args.package:
        packer["com.prpr.musedash"]["package"] = args.package
    if args.master:
        packer["com.prpr.musedash"]["hex"][0]["patch"].append(PA_MASTER)
    ...
    if apk_dir is None:
        ...
        if ctx is not None:
            ...
            click.echo(main.get_help(click.RichContext(main)))
        sys.exit(0)
