...
void printColored(const std::string& text, Color color, bool bold = false) {
    std::cout << "\033[" << (bold ? "1;" : "") << color << "m" << text << "\033[0m" << std::endl;
}

// Function to convert bytes to a formatted hex string for display
std::string bytesToHexString(const std::vector<uint8_t>& bytes, bool colored = true, bool allowWildcards = true) {
    std::ostringstream oss;
    for (size_t i = 0; i < bytes.size(); ++i) {
        if (i > 0) oss << " "; // Single space between bytes
        if (colored && allowWildcards && bytes[i] == 0x00) {
            oss << "\033[1;34m??\033[0m";  // Bold blue for wildcards
        } else {
            oss << "\033[34m" << std::hex << std::uppercase << std::setw(2) << std::setfill('0') 
                << static_cast<int>(bytes[i]) << "\033[0m";
        }
    }
    return oss.str();
}
